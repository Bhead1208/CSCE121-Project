...
struct Lines_window : Graph_lib::Window {       // inherits from Window
  ...
  Lines_window(Point xy,             // top lefthand corner
	...
	void addPoint(Lines_window*,Lines_window*,Lines_window*,Lines_window*);
  ...
  // body of constructor follows
{
  ...
  menu.attach(new Button(Point(0,0),0,0,"Transfer",cb_transfer));
  attach(menu);
  menu.hide();
...
void Lines_window::next() {
  ...
  switch(option)
  {
	  ...
	  {
		  //x is the id
		  //y is the currency type of the to bank
		  //z is the amount to transfer
		  //find the other bank
		  string str2 = y.substr(0,3);
		  bool isBank = false;
		  BankIntl* otherBank;
		  for (int i = 0; i < others.size() && !isBank; i++)
		  {
			  cout << others.at(i)->myBank.cash.getCurrency().type << endl;
			  cout << y << endl;
			  string str1 = others.at(i)->myBank.cash.getCurrency().type.substr(0,3);
			  
			  bool test = (others.at(i)->myBank.cash.getCurrency().type.compare(y)==0);
			  cout << str1.compare(str2) << endl;
			  if (str1.compare(str2)==0)
			  {
				  isBank = true;
				  otherBank = &(others.at(i)->myBank);
			  }
		  }
		  if (!isBank)
		  {
			  out1.put("No Bank found");
		  }
		  else
		  {
			  if (myBank.findPatron(stoi(x)) && otherBank->findPatron(stoi(x)))
			  {
					Vector<Patron> pats = myBank.getPats();
					int index;
					for(int i = 0; i < pats.size(); i++){
						if(pats[i].get_idNum() == stoi(x)){
							index = i;
							break;
						}
					}
					if (pats[index].get_Money() < stod(z))
					{
						out1.put("Overdraw Error");
					}
					else
					{
						myBank.withdrawal(stoi(x),myBank.cash.getCurrency().type,stod(z));
						otherBank->deposit(stoi(x),myBank.cash.getCurrency().type,stod(z));
					}
			  } 
			  else
			  {
...
void Lines_window::cb_transfer(Address, Address pw) {
  reference_to<Lines_window>(pw).transfer();   // transfer is defined next
}

//------------------------------------
// what to do when transfer button is pressed
...
void Lines_window::addPoint(Lines_window* la, Lines_window* lb, Lines_window* lc, Lines_window* ld)
{
	others.push_back(la);
	others.push_back(lb);
	others.push_back(lc);
...
int main() 
  try {
	...
	Lines_window* e;
	e = &win5;
	win1.addPoint(b,c,d,e);
	win2.addPoint(a,c,d,e);
	win3.addPoint(b,a,d,e);
	win4.addPoint(b,c,a,e);
	win5.addPoint(b,c,d,a);
